...
use App\SalesBill;
use App\SalesBillDetails;
use App\SalesBillDetailSrials;
...
class SalesBillRepository
{
     ...
     * @return void
     */
    public function __construct(SalesBill $sales_bill)
    {
        $this->data = $sales_bill->with('user', 'user.profile', 'customer', 'branch', 'SalesBillDetails', 'SalesBillDetails.SalesBillDetailSrials');
    }
    ...
    public function create($params)
    {
        ...
        for ($i = 0; $i < count($params['products']); $i++) {
            $datadetail = new SalesBillDetails();
            $datadetail->product_id = $params['products'][$i]['id'];
            $datadetail->amount = $params['products'][$i]['price'];
            $datadetail->user_id = auth()->user()->id;
            $datadetail = $data->SalesBillDetails()->save($datadetail);
            // will save the image for the post.
            for ($k = 0; $k < count($params['products'][$i]['serial']); $k++) {
                $SalesBillDetailSrials = new SalesBillDetailSrials();
                $SalesBillDetailSrials->srialnumper = $params['products'][$i]['serial'][$k];
                $SalesBillDetailSrials->user_id = auth()->user()->id;
                $datadetail->SalesBillDetailSrials()->save($SalesBillDetailSrials); // will save the image for the post.
            }
        }
        return $data;
    }
     ...
     * @return branch
     */
    public function update(SalesBill $sales_bill, $params)
    {
        $sales_bill->forceFill($this->formatParams($params, 'update'))->save();
        return $sales_bill;
    }
     ...
     * @return bool|null
     */
    public function delete(SalesBill $sales_bill)
    {
        return $sales_bill->delete();
    }
