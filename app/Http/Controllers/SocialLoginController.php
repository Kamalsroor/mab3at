    ...
    public function providerRedirectCallback($provider = '')
    {
        ...
        if ($user_exists) {
            ...
            if ($user_exists->status === 'pending_activation') {
                \Cache::put('message', trans('auth.pending_activation'), 60);
            }

            if ($user_exists->status === 'pending_approval') {
                \Cache::put('message', trans('auth.pending_approval'), 60);
            }
            ...
            if ($user_exists->status === 'banned') {
                \Cache::put('message', trans('auth.account_banned'), 60);
            }
