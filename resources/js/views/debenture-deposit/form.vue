...
export default {
  ...
  mounted() {
    ...
    if (this.id) this.get();
    axios
      .get("/api/debenture_deposit/pre-requisite")
  ...
  methods: {
    ...
    getCustomerAccount() {
      ...
      axios
        ...
        .catch(error => {
          helper.showDataErrorMsg(error);
          this.$router.push("/debenture_deposit");
        });
    },
    get() {
      axios
        .get("/api/debenture_deposit/" + this.id)
        .then(response => response.data)
        .then(response => {
    ...
    update() {
      this.generalForm
        .patch("/api/debenture_deposit/" + this.id)
        .then(response => {
          toastr.success(response.message);
          this.$router.push("/debenture_deposit");
        })
