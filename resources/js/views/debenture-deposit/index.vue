  ...
  <div>
    <div class="page-titles p-3 border-bottom">
      <h3 class="text-themecolor">
        {{trans('debenture_cashing.debenture_deposit')}}
        <span
          class="card-subtitle"
          v-if="debenture_deposits"
        >{{trans('general.total_result_found',{'count' : debenture_deposits.total})}}</span>
        ...
        <button
          class="btn btn-info btn-sm pull-right m-r-10"
          v-if="debenture_deposits.total && !showCreatePanel"
    ...
    <div class="container-fluid p-0">
      <div class="row">
        <div class="col-12">
          ...
          <div class="card">
            <div class="card-body">
              <div class="table-responsive" v-if="debenture_deposits.total">
                <table class="table">
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    <tr v-for="(debenture_deposit, index) in debenture_deposits.data" :key="index">
                      <td
                        v-text="debenture_deposit.user.profile.first_name+' '+debenture_deposit.user.profile.last_name"
                      ></td>
                      <td v-text="debenture_deposit.customer.name"></td>
                      <td v-text="debenture_deposit.branch.name"></td>
                      <td v-text="debenture_deposit.amount"></td>
                      <td v-text="debenture_deposit.date_at"></td>
                      <!-- <td v-text="debenture_deposit.name"></td>

                      <td v-text="debenture_deposit.group.name"></td>
                      <td v-text="debenture_deposit.max_price"></td>
                      <td v-text="debenture_deposit.min_price"></td>
                      <td v-text="debenture_deposit.parcode"></td>-->
                      <!-- <td v-text="debenture_deposit.status == 1 ? 'Active' : 'Not Active'"></td> -->
                      <td class="table-option">
                        <div class="btn-debenture_deposit">
                          <button
                            class="btn btn-info btn-sm"
                            v-tooltip="trans('general.edit')"
                            @click.prevent="edit(debenture_deposit)"
                          >
                            <i class="fas fa-edit"></i>
                          </button>
                          <button
                            class="btn btn-danger btn-sm"
                            :key="debenture_deposit.id"
                            v-confirm="{ok: confirmDelete(debenture_deposit)}"
...
export default {
  ...
  methods: {
    ...
    getCustomers(page) {
      ...
      axios
        .get("/api/debenture_deposit?page=" + page)
        .then(response => response.data)
        .then(response => (this.debenture_deposits = response))
        .catch(error => {
          helper.showDataErrorMsg(error);
        });
    },
    edit(debenture_deposit) {
      this.$router.push("/debenture_deposit/" + debenture_deposit.id + "/edit");
    },
    confirmDelete(debenture_deposit) {
      return dialog => this.delete(debenture_deposit);
    },
    delete(debenture_deposit) {
      axios
        .delete("/api/debenture_deposit/" + debenture_deposit.id)
        .then(response => response.data)
        .then(response => {
