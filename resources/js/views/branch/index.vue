<template>
    <div>
        <div class="page-titles p-3 border-bottom">
            <h3 class="text-themecolor">{{trans('branch.branch')}} 
                <span class="card-subtitle" v-if="branches">{{trans('general.total_result_found',{'count' : branches.total})}}</span>
                ...
                <button class="btn btn-info btn-sm pull-right m-r-10" v-if="branches.total && !showCreatePanel" @click="showCreatePanel = !showCreatePanel"><i class="fas fa-plus"></i> <span class="d-none d-sm-inline">{{trans('general.add_new')}}</span></button>
            </h3>
        </div>
                ...
                <div class="col-12">
                    ...
                    <div class="card">
                        <div class="card-body">
                                    ...
                                    <tbody>
                                        <tr v-for="(branch, index) in branches.data" :key="index" >
                                            ...
                                            <td class="table-option">
                                                <div class="btn-group">
                                                    <button class="btn btn-info btn-sm" v-tooltip="trans('general.edit')" @click.prevent="edit(branch)"><i class="fas fa-edit"></i></button>
                                                    <button class="btn btn-danger btn-sm" :key="branch.id" v-confirm="{ok: confirmDelete(branch)}" v-tooltip="trans('general.delete')"><i class="fas fa-trash"></i></button>
                                                </div>
                                            </td> 
                                        </tr>
                                    </tbody>
                            ...
                            <module-info v-if="!branches.total" module="todo" title="module_info_title" description="module_info_description" icon="check-circle">
                                <div slot="btn">
                                    <button class="btn btn-info btn-md" v-if="!showCreatePanel" @click="showCreatePanel = !showCreatePanel"><i class="fas fa-plus"></i> {{trans('general.add_new')}}</button>
                                </div>
                            </module-info>
                            <pagination-record :page-length.sync="filterTodoForm.page_length" :records="branches" @updateRecords="getBranches"></pagination-record>
                        </div>
                    </div>
                </div>
            </div>
