    ...
    <div class="page-titles p-3 border-bottom">
      <h3 class="text-themecolor">
        ...
        <span
          class="card-subtitle"
          v-if="permissions"
        >{{trans('general.total_result_found',{'count' : permissions.total})}}</span>
        <span class="card-subtitle" v-else>{{trans('general.no_result_found')}}</span>
        <button class="btn btn-info btn-sm pull-right" @click="$router.push('/home')">
          <i class="fas fa-home"></i>
          <span class="d-none d-sm-inline">{{trans('general.home')}}</span>
        </button>
        <button
        ...
        >
          ...
          <span class="d-none d-sm-inline">{{trans('permission.add_new_permission')}}</span>
        </button>
        <button
          class="btn btn-info btn-sm pull-right m-r-10"
          @click="$router.push('/configuration/permission/assign')"
        >
          <i class="fas fa-user-plus"></i>
          <span class="d-none d-sm-inline">{{trans('permission.assign_permission')}}</span>
        </button>
      </h3>
    </div>
                ...
                <table class="table">
                  ...
                  <tbody>
                    <tr v-for="permission in permissions.data">
                      <td v-text="toWord(permission.label)"></td>
...
export default {
  ...
  methods: {
    getPermissions(page) {
      ...
      axios
        .get("/api/permission?page=" + page + url)
