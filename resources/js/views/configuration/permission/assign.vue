    ...
    <div class="page-titles p-3 border-bottom">
      <h3 class="text-themecolor">
        ...
        <button
          ...
          @click="$router.push('/configuration/permission')"
        >
          <i class="fas fa-key"></i>
          <span class="d-none d-sm-inline">{{trans('permission.permission')}}</span>
        </button>
      </h3>
    </div>
        ...
        <div class="col-sm-12">
          <div class="card">
            <div class="card-body">
                ...
                <table class="table table-hover">
                  ...
                  <tbody>
                    <tr v-for="permission in permissions">
                      <td>{{toWord(permission.label)}}</td>
                      <td v-for="role in roles" class="text-center">
                        <switches
                          v-model="assignPermissionForm.data[role.id][permission.id]"
                          theme="bootstrap"
                          color="success"
                        ></switches>
                      </td>
              ...
              <button
                type="button"
                class="btn btn-sm btn-info waves-effect waves-light m-l-20"
                @click="savePermission"
              >{{trans('general.save')}}</button>
              <router-link
                to="/configuration/permission"
                class="btn btn-sm btn-danger"
              >{{trans('general.back')}}</router-link>
            </div>
          </div>
        </div>
      </div>
