  ...
  <div>
    <div class="page-titles p-3 border-bottom">
      <h3 class="text-themecolor">
        {{trans('purchases_bill.purchases_bill')}}
        <span
          class="card-subtitle"
          v-if="purchases_bills"
        >{{trans('general.total_result_found',{'count' : purchases_bills.total})}}</span>
        ...
        <button
          class="btn btn-info btn-sm pull-right m-r-10"
          v-if="purchases_bills.total && !showCreatePanel"
    ...
    <div class="container-fluid p-0">
      <div class="row">
        <div class="col-12">
          ...
          <div class="card">
            <div class="card-body">
              <div class="table-responsive" v-if="purchases_bills.total">
                <table class="table">
                  <thead>
                    <tr>
                      ...
                      <th>{{trans('purchases_bill.max_price')}}</th>
                      <th>{{trans('purchases_bill.min_price')}}</th>
                      <th>{{trans('purchases_bill.parcode')}}</th>
                      <th>{{trans('purchases_bill.status')}}</th>-->
                      <th class="table-option">{{trans('general.action')}}</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr v-for="(purchases_bill, index) in purchases_bills.data" :key="index">
                      <td
                      ...
                      <td v-text="purchases_bill.max_price"></td>
                      <td v-text="purchases_bill.min_price"></td>
                      <td v-text="purchases_bill.parcode"></td>-->
                      <!-- <td v-text="purchases_bill.status == 1 ? 'Active' : 'Not Active'"></td> -->
                      <td class="table-option">
                        <div class="btn-purchases_bill">
                          <button
                            class="btn btn-info btn-sm"
                            v-tooltip="trans('general.edit')"
                            @click.prevent="edit(purchases_bill)"
                          >
                            <i class="fas fa-edit"></i>
                          </button>
                          <button
                            class="btn btn-danger btn-sm"
                            :key="purchases_bill.id"
                            v-confirm="{ok: confirmDelete(purchases_bill)}"
...
export default {
  ...
  methods: {
    ...
    getCustomers(page) {
      ...
      axios
        .get("/api/purchases_bill?page=" + page)
        .then(response => response.data)
        .then(response => (this.purchases_bills = response))
        .catch(error => {
          helper.showDataErrorMsg(error);
        });
    },
    edit(purchases_bill) {
      this.$router.push("/purchases_bill/" + purchases_bill.id + "/edit");
    },
    confirmDelete(purchases_bill) {
      return dialog => this.delete(purchases_bill);
    },
    delete(purchases_bill) {
      axios
        .delete("/api/purchases_bill/" + purchases_bill.id)
        .then(response => response.data)
        .then(response => {
